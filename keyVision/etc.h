...
protected:
	...
	mutable std::vector<std::array<sf::Sprite, 10>> _numSprites;
public:
	Number() = delete;
	Number(int type, size_t digit) : sf::Sprite(), numType(type) { _numSprites.resize(digit); }
	Number(int type, size_t digit,
		const sf::Texture &texture, unsigned div_x = 1, unsigned div_y = 1)
		: numType(type), sf::Sprite(texture), div_x(div_x), div_y(div_y)
	{
